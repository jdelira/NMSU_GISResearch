     //ENTITY TYPE ID
      ...
      function findLocation()
      {
                ...
                var USCensusSearch = "http://geocoding.geo.census.gov/geocoder/locations/onelineaddress?address=" + address + "&benchmark=Public_AR_Current" + "&format=jsonp" + "&callback=getUSAddressRange"; 
                //set its source api to the JSONP API
                mapscript2.type = 'text/javascript'; 
                mapscript2.src = USCensusSearch;
    ...
    function findListHomes(a)
    {
      ...
      function getUSAddressRange(json)
      {   
        //console.log(json.result.input.benchmark.benchmarkName);
        //console.log(json.result.input.address.address);
        
          if(json)
          {
              //returns two properties: input and matchedAddresses with properties of their own
              for(var prop in json.result.addressMatches) 
              {
                console.log(json.result.addressMatches["coordinates"]); 
              }
          }
      }
